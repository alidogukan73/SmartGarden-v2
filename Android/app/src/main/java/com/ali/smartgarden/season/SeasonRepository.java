...
public final class SeasonRepository {
    ...
    private void writeSeasonManifest(
            MutableData root,
            String seasonId,
            GardenZone zone,
            MutableData zoneData,
            String plantingDate,
            String status,
            long startedAt,
            long endedAt,
        ...
        MutableData season = root.child("garden_journal").child("seasons").child(seasonId);
        season.child("season_id").setValue(seasonId);
        season.child("zone_id").setValue(zone.getZone_id());
        season.child("zone_name").setValue(stringValue(zoneData.child("name")));
        season.child("plant_type").setValue(stringValue(zoneData.child("plant_type")));
        season.child("emoji").setValue(stringValue(zoneData.child("emoji")));
        season.child("sensor_id").setValue(stringValue(zoneData.child("sensor_id")));
        season.child("sensor_enabled").setValue(booleanValue(zoneData.child("sensor_enabled")));
        season.child("valve_id").setValue(stringValue(zoneData.child("valve_id")));
        season.child("valve_mode").setValue(stringValue(zoneData.child("valve_mode")));
        season.child("label").setValue(seasonLabel(zone, startedAt));
        season.child("status").setValue(status);
        season.child("planting_date").setValue(safe(plantingDate));
        season.child("started_at_epoch").setValue(startedAt);
        season.child("ended_at_epoch").setValue(endedAt);
        season.child("includes_legacy_records").setValue(includesLegacy);
        season.child("created_at_epoch").setValue(now);
        season.child("updated_at_epoch").setValue(now);
    }

    private static void writeSeasonState(
            MutableData state,
            String seasonId,
            String status,
            String label,
        ...
        state.child("status").setValue(status);
        state.child("label").setValue(label);
        state.child("started_at_epoch").setValue(startedAt);
        state.child("ended_at_epoch").setValue(endedAt);
        state.child("include_legacy_records").setValue(includeLegacy);
        state.child("updated_at_epoch").setValue(now);
    }

    private static void clearStaleSensorMeasurement(MutableData zoneData) {
        zoneData.child("moisture").setValue(0);
        zoneData.child("raw").setValue(0);
        zoneData.child("voltage").setValue(0.0d);
        zoneData.child("rssi").setValue(0);
        zoneData.child("updated_at_epoch").setValue(0L);
    }

    private static void resetSeasonalIrrigationState(MutableData irrigation, long now) {
        irrigation.child("decision").setValue("WAIT");
        irrigation.child("decision_reason").setValue("NEW_SEASON_STARTED");
        irrigation.child("cooldown_active").setValue(false);
        irrigation.child("cooldown_remaining").setValue(0);
        irrigation.child("queue_position").setValue(0);
        irrigation.child("selected_for_watering").setValue(false);
        irrigation.child("watering_active").setValue(false);
        irrigation.child("completed_watering_cycles").setValue(0);
        irrigation.child("waiting_for_moisture_recovery").setValue(false);
        irrigation.child("updated_at_epoch").setValue(now);
    }

    /**
     * Keeps the season's final agronomic state for later comparison without
     * moving or deleting any live zone data. Hardware identity and secrets are
     * intentionally excluded from the archive.
     */
    private static void snapshotSeasonState(MutableData snapshots, MutableData zoneData, long now) {
        snapshots.child("ai").setValue(zoneData.child("ai").getValue());
        snapshots.child("irrigation_status").setValue(
                zoneData.child("irrigation_status").getValue());
        snapshots.child("fertilization").setValue(
                zoneData.child("fertilization").getValue());

        MutableData settings = snapshots.child("zone_settings");
        copyValue(settings, zoneData, "area_m2");
        copyValue(settings, zoneData, "tank_volume_l");
        copyValue(settings, zoneData, "moisture_limit");
        copyValue(settings, zoneData, "pump_duration");
        copyValue(settings, zoneData, "cooldown_seconds");
        copyValue(settings, zoneData, "restart_delta");
        settings.child("captured_at_epoch").setValue(now);
    }

    private static void copyValue(MutableData target, MutableData source, String key) {
        target.child(key).setValue(source.child(key).getValue());
    }

    private static void copyOutcome(
            MutableData target,
            SeasonOutcome outcome,
            String seasonId,
            String zoneId,
        ...
        target.child("season_id").setValue(seasonId);
        target.child("zone_id").setValue(zoneId);
        target.child("result").setValue(outcome == null ? "" : outcome.getResult());
        target.child("harvest_amount").setValue(outcome == null ? "" : outcome.getHarvest_amount());
        target.child("yield_note").setValue(outcome == null ? "" : outcome.getYield_note());
        target.child("issues_note").setValue(outcome == null ? "" : outcome.getIssues_note());
        target.child("successful_practices").setValue(outcome == null ? "" : outcome.getSuccessful_practices());
        target.child("next_season_note").setValue(outcome == null ? "" : outcome.getNext_season_note());
        target.child("water_summary").setValue(outcome == null ? "" : outcome.getWater_summary());
        target.child("fertilizer_summary").setValue(outcome == null ? "" : outcome.getFertilizer_summary());
        target.child("recorded_at_epoch").setValue(now);
    }
    ...
    private static ZoneSeasonState readSeasonState(MutableData state) {
        ZoneSeasonState value = new ZoneSeasonState();
        value.setActive_season_id(stringValue(state.child("active_season_id")));
        value.setStatus(stringValue(state.child("status")));
        value.setLabel(stringValue(state.child("label")));
        value.setStarted_at_epoch(longValue(state.child("started_at_epoch")));
        value.setEnded_at_epoch(longValue(state.child("ended_at_epoch")));
        value.setInclude_legacy_records(booleanValue(state.child("include_legacy_records")));
        value.setUpdated_at_epoch(longValue(state.child("updated_at_epoch")));
        return value;
    }
